...
def view_all_guests(request):
    ...
    return render(request, "guests/guests.html.jinja", context)
...
def guest_detail(request, actors_id):
    ...
    return render(request, "guests/guest_detail.html.jinja", context)
...
def add_guest(request):
    ...
    if not request.user.is_superuser:
        messages.error(
            request,
            "Sorry, you don't have permission to add \
    ...
    template = "guests/add_guest.html.jinja"
    context = {
        "form": form,
    }
...
def edit_guest_info(request, actors_id):
    ...
    if not request.user.is_superuser:
        messages.error(
            request,
            "Sorry, you don't have permission to edit \
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            messages.error(
                request,
                "Guest info could not be updated! \
                Please check the form input.",
            )
    else:
        # avoid wiping out the form errors
    ...
    template = "guests/edit_guest_info.html.jinja"
    context = {
        "form": form,
        "actor": actor,
    }
